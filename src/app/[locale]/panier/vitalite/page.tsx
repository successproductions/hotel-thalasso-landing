...
export default function VitalitePanierPage() {
  ...
  const currentOffer = {
    title: locale === 'fr' ? "THALASSO VITALITÉ 3 JOURS" : "THALASSO VITALITY 3 DAYS",
    subtitle: locale === 'fr' ? "Escapade Bien-être" : "Wellness Getaway",
    includes: locale === 'fr'
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Format matches the prefix logic in /api/payment/initiate (VIT = vitalite, offer 3)
      const timestamp = Date.now().toString(36);
      const randomPart = Math.random().toString(36).substring(2, 8);
      const orderId = `VIT3-${timestamp}${randomPart}`.toUpperCase();
  ...
  return (
    <div className="min-h-screen bg-white flex flex-col font-sans">
      ...
      <header className="border-b border-gray-100 bg-white py-4 px-6 md:px-12 flex justify-between items-center z-10 sticky top-0">
        ...
        <a href={`/${locale}/${'vitalite'}`} className="text-sm font-medium text-gray-500 hover:text-gray-900 transition-colors flex items-center gap-2">
