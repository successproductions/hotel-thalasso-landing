...
export default function BookingFormPopup({ isOpen, onClose }: BookingFormPopupProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await fetch('/api/reservations/evasion', {
        method: 'POST',
  ...
  const totalAmount = (price * parseInt(formData.numberOfPeople || '1')).toLocaleString('fr-FR');
  const ctaLabel = locale === 'fr'
    ? `Confirmer la réservation - ${totalAmount} MAD`
    : `Confirm Reservation - ${totalAmount} MAD`;
        ...
        <div className="lg:grid lg:grid-cols-2">
          ...
          <div className="hidden lg:flex lg:flex-col">
            <div className="relative h-[280px] w-full overflow-hidden">
              <Image
                src="/images/THERMALE.png"
                alt="Thalasso Vitalité"
                fill
            ...
            <div className="flex-1 bg-[#faf9f5] p-6 flex flex-col justify-center">
              <span className="text-sm uppercase tracking-wider text-[#d6bb8e] font-medium">
                {locale === 'fr' ? 'Escapade Bien-être' : 'Wellness Getaway'}
              </span>
              <h3 className="mt-2 text-2xl font-light text-gray-900">
                {locale === 'fr' ? 'THALASSO VITALITÉ 3 JOURS' : 'THALASSO VITALITY 3 DAYS'}
              </h3>
              ...
              <p className="mt-2 text-xs italic text-gray-500">
                ...
                {locale === 'fr' ? "Ce pack n'inclut pas l'hébergement" : "This pack does not include accommodation"}
              </p>
          ...
          <div className="p-3 lg:p-6">
            ...
            <div className="lg:hidden relative h-36 w-full overflow-hidden mb-4 -mx-5 -mt-5" style={{ width: 'calc(100% + 2.5rem)' }}>
              {bannerImages.map((image, index) => (
                ...
                >
                  <Image src={image} alt={`Dakhla Club - Image ${index + 1}`} fill className="object-cover" priority={index === 0} />
                </div>
              ))}
            ...
            <form onSubmit={handleSubmit} className="space-y-2 lg:space-y-4">
              ...
              <div className="flex items-center space-x-2 my-4">
                ...
                <label htmlFor="acceptTerms" className="text-sm text-gray-600">
                  {t('terms.agree')} <a href="/fr/legal/cgv" target="_blank" className="underline hover:text-[#139584]">{t('terms.link')}</a>
                </label>
              </div>
