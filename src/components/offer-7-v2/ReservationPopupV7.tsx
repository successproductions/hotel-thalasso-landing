...
interface ReservationPopupV7Props {
  isOpen: boolean;
  onClose: () => void;
}
...
export default function ReservationPopupV7({ isOpen, onClose }: ReservationPopupV7Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Send confirmation email via API - using offer7 endpoint
      const emailResponse = await fetch('/api/reservations/offer7', {
        method: 'POST',
        headers: {
      ...
      // Redirect to evasion-7 payment page
      window.location.href = `/${locale}/evasion-7/payment`;
    } catch (error) {
