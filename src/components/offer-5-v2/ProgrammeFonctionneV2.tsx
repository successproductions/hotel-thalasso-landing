...
export function ProgrammeFonctionneV2() {
  const t = useTranslations('offer5.ProgrammeFonctionne.points');
  const te = useTranslations('offer5.ProgrammeFonctionne');
    ...
    <section id="pourquoi" className="bg-white md:py-12">
      <div className="max-w-8xl mx-auto grid grid-cols-1 gap-6 md:grid-cols-2 md:gap-44">
        ...
        <div className="space-y-6">
          <h2 className="block pl-2 md:text-center text-[27px] font-normal text-gray-800 md:hidden md:pt-14 md:text-4xl">
            {te('title2')}
          </h2>
          <h2 className="hidden pt-6 text-3xl font-normal text-gray-800 md:block md:pt-14 md:text-4xl">
            {te('title2').toUpperCase()}
          </h2>
          <ul className="list-disc space-y-3 pl-6 text-base font-light leading-relaxed text-gray-600 md:text-lg">
            {[0, 1, 2, 3, 4, 5].map((i) => {
              const point = t(`${i}`);
