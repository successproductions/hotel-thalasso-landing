...
const generatePage = (slug, data) => `'use client';
...
export default function ${data.componentName}() {
  ...
  const currentOffer = {
    title: locale === 'fr' ? ${JSON.stringify(data.titleFr)} : ${JSON.stringify(data.titleEn)},
    subtitle: locale === 'fr' ? ${JSON.stringify(data.subtitleFr)} : ${JSON.stringify(data.subtitleEn)},
    includes: locale === 'fr'
      ? ${JSON.stringify(data.includesFr)}
      : ${JSON.stringify(data.includesEn)},
    image: ${JSON.stringify(data.image)},
    basePriceMAD: ${data.basePriceMAD},
  ...
  return (
    <div className="min-h-screen bg-white flex flex-col font-sans">
      ...
      <header className="border-b border-gray-100 bg-white py-4 px-6 md:px-12 flex justify-between items-center z-10 sticky top-0">
        ...
        <a href={\`/\${locale}/\${'${data.returnSlug}'}\`} className="text-sm font-medium text-gray-500 hover:text-gray-900 transition-colors flex items-center gap-2">
          ...
          <div className="max-w-xl mx-auto lg:max-w-none lg:sticky lg:top-28">
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 space-y-4">
              <div className="flex justify-between text-sm text-gray-600">
                ...
                <span className="font-medium text-gray-900">{basePriceMAD.toLocaleString('fr-FR')} MAD</span>
              </div>
              <div className="flex justify-between text-sm text-gray-600">
...
const data = {
  vitalite: {
    componentName: 'VitalitePanierPage',
    offerId: '3',
    returnSlug: 'vitalite',
    titleFr: 'THALASSO VITALITÉ 3 JOURS',
    titleEn: 'THALASSO VITALITY 3 DAYS',
    subtitleFr: 'Escapade Bien-être',
    subtitleEn: 'Wellness Getaway',
  ...
  regeneration: {
    ...
    offerId: '5',
    returnSlug: 'regeneration',
    titleFr: 'THALASSO RÉGÉNÉRATION 5 JOURS',
    titleEn: 'THALASSO REGENERATION 5 DAYS',
    subtitleFr: 'Retraite Revitalisante',
    subtitleEn: 'Revitalizing Retreat',
  ...
  renaissance: {
    ...
    offerId: '7',
    returnSlug: 'renaissance',
    titleFr: 'THALASSO RENAISSANCE 7 JOURS',
    titleEn: 'THALASSO RENAISSANCE 7 DAYS',
    subtitleFr: 'Cure Holistique',
    subtitleEn: 'Holistic Cure',
  ...
  evasion: {
    ...
    offerId: '3',
    returnSlug: 'evasion-holistique-3-jours',
    titleFr: 'THALASSO ÉVASION 3 JOURS',
    titleEn: 'THALASSO EVASION 3 DAYS',
    subtitleFr: 'Évasion Holistique',
    subtitleEn: 'Holistic Escape',
    includesFr: ['14 soins thalasso', '423 minutes de soins', '8 soins softs et 6 hards', 'Accès spa et piscine'],
...
for (const [slug, opts] of Object.entries(data)) {
  fs.writeFileSync(path.join(__dirname, 'src/app/[locale]/panier', slug, 'page.tsx'), generatePage(slug, opts));
}
console.log("SUCCESS");
